...
class CreditItemsRepository {
  ///
  IsarCollection<CreditItem> getCollection({required Isar isar}) =>
      isar.creditItems;

  ///
  Future<CreditItem?> getCreditItem(
  ...
  ///
  Future<void> inputCreditItemList(
      {required Isar isar, required List<CreditItem> creditItemList}) async {
    for (final CreditItem element in creditItemList) {
      inputCreditItem(isar: isar, creditItem: element);
    }
  }

  ///
  Future<void> inputCreditItem(
      {required Isar isar, required CreditItem creditItem}) async {
    final IsarCollection<CreditItem> creditItemsCollection =
        getCollection(isar: isar);
